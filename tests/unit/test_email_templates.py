...
from app.services.email_service import SAFE_USER_REPLY_FALLBACK, ResendEmailService
...
@readable_test_id("письмо пользователю рендерится без внутренней классификации")
def test_user_confirmation_is_rendered_without_internal_classification(_case_id) -> None:
    """EMAIL-RENDER-USER-001: письмо пользователю не раскрывает AI-классификацию."""
    message = ResendEmailService(settings=make_settings()).build_user_message(make_context(ai_error="provider failed"))

    assert message.html
    assert message.text
    assert "Иван Иванов" in message.html
    assert "Спасибо за обращение" in message.text
    assert "project_request" not in message.html
    assert "provider failed" not in message.text
...
@readable_test_id("suggested reply пользователя экранируется")
def test_user_suggested_reply_is_escaped(_case_id) -> None:
    ...
    message = ResendEmailService(settings=make_settings()).build_user_message(context)

    assert "<script>" not in message.html
    assert "&lt;script&gt;" in message.html


@readable_test_id("fallback отображается владельцу и безопасен пользователю")
def test_ai_fallback_is_rendered_safely(_case_id) -> None:
    """EMAIL-RENDER-FALLBACK-001: fallback виден владельцу, но не раскрывает технику пользователю."""
    ...
    assert SAFE_USER_REPLY_FALLBACK in user_message.text
    assert "OpenAI timeout" not in user_message.html
