...
| README-DOC-001 | README описывает фактическую owner-only email логику | Manual | Нет | Review README | [x] |
| README-DOC-002 | README содержит только реальные CLI-команды | Manual | Нет | `python -m app.cli --help` | [x] |
| README-DOC-003 | README не содержит секреты и реальные локальные значения | Manual | Нет | Review README и grep-проверка | [x] |
| README-DOC-004 | README не обещает деплой и user autoreply | Manual | Нет | Review README | [x] |

## Финальный checklist перед сдачей проекта

- [x] Все автоматические тесты пройдены.
- [x] Ручной health check выполнен.
- [x] Миграции применяются из чистого состояния.
- [x] AI live-вызов через ProxyAPI выполнен только при явном разрешении.
- [x] Реальное тестовое письмо отправлено только при явном разрешении.
- [x] Секреты отсутствуют в README и `.env.example`.
- [x] README финального этапа описывает запуск, API, деплой и ограничения.
